...
class F09MineSecondaryPagesIntegrationTest {
 ...
 @Test void mineFriendsPrivacyAndSchedulesFormClosedLoops() throws Exception {
  ...
  mvc.perform(put("/api/admin/users/"+a.id+"/ai-authorization").header("X-Admin-Token","dev-admin-token").contentType(MediaType.APPLICATION_JSON).content("{\"enabled\":true}")).andExpect(status().isOk());
