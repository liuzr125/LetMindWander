...
@ActiveProfiles("dev") @AutoConfigureMockMvc class F04DailyTaskIntegrationTest {
 ...
 @Test void recordsStateEventsAndComputesServerProgress() throws Exception {String token=register();mvc.perform(post("/api/plans/default").header("Authorization","Bearer "+token)).andExpect(status().isOk());LocalDate businessDate=LocalDate.now(ZoneId.of("Asia/Shanghai")).plusDays(1);while(businessDate.getDayOfWeek().getValue()>5)businessDate=businessDate.plusDays(1);String date=businessDate.toString();String day=mvc.perform(get("/api/days/{date}",date).header("Authorization","Bearer "+token)).andExpect(status().isOk()).andExpect(jsonPath("$.active").value(true)).andExpect(jsonPath("$.currentCount").value(1)).andReturn().getResponse().getContentAsString();String id=json.readTree(day).path("tasks").get(0).path("id").asText();mvc.perform(post("/api/tasks/{id}/events",id).header("Authorization","Bearer "+token).contentType(MediaType.APPLICATION_JSON).content("{\"eventType\":\"start\",\"expectedVersion\":1}")).andExpect(status().isOk()).andExpect(jsonPath("$.status").value("DOING"));mvc.perform(post("/api/tasks/{id}/events",id).header("Authorization","Bearer "+token).contentType(MediaType.APPLICATION_JSON).content("{\"eventType\":\"complete\",\"expectedVersion\":2}")).andExpect(status().isOk()).andExpect(jsonPath("$.status").value("DONE"));mvc.perform(get("/api/days/{date}",date).header("Authorization","Bearer "+token)).andExpect(jsonPath("$.doneCount").value(1)).andExpect(jsonPath("$.completionRate").value(100.0));}
