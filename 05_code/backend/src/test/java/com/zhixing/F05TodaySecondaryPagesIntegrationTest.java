...
class F05TodaySecondaryPagesIntegrationTest {
    ...
    @Test void todayContentJournalAndReviewFormAClosedLoop() throws Exception {
        ...
        seedBook();
        mvc.perform(put("/api/vocabulary-books/current").header("Authorization",bearer(session.token))
                .contentType(MediaType.APPLICATION_JSON).content("{\"bookId\":\"12121212121212121212121212121212\",\"dailyNewLimit\":3}"))
                .andExpect(status().isOk()).andExpect(jsonPath("$.selected").value(true));
        String today=LocalDate.now(ZoneId.of("Asia/Shanghai")).toString();
        seedAction(session.userId,LocalDate.parse(today));
        ...
        JsonNode tech=find(day,"tech"),word=find(day,"word"),journal=find(day,"journal"),action=find(day,"action");
        mvc.perform(get("/api/actions/{id}",action.path("actionId").asText()).header("Authorization",bearer(session.token)))
                .andExpect(status().isOk()).andExpect(jsonPath("$.title").value("整理今日笔记"));
        mvc.perform(put("/api/actions/{id}",action.path("actionId").asText()).header("Authorization",bearer(session.token))
                .contentType(MediaType.APPLICATION_JSON).content("{\"title\":\"整理 RAG 笔记\",\"note\":\"写出三条要点\",\"estimatedMinutes\":2,\"expectedVersion\":1}"))
                .andExpect(status().isOk()).andExpect(jsonPath("$.versionNo").value(2)).andExpect(jsonPath("$.estimatedMinutes").value(2));
        mvc.perform(post("/api/tasks/{id}/events",action.path("id").asText()).header("Authorization",bearer(session.token))
                .contentType(MediaType.APPLICATION_JSON).content("{\"eventType\":\"start\",\"expectedVersion\":2}")).andExpect(status().isOk()).andExpect(jsonPath("$.status").value("DOING"));
        mvc.perform(post("/api/tasks/{id}/events",action.path("id").asText()).header("Authorization",bearer(session.token))
                .contentType(MediaType.APPLICATION_JSON).content("{\"eventType\":\"complete\",\"expectedVersion\":3}")).andExpect(status().isOk()).andExpect(jsonPath("$.status").value("DONE"));
    ...
    private void seedBook(){jdbc.update("INSERT INTO vocabulary_book(id,book_code,book_name,book_type,level_code,description,word_count,sort_no,is_recommended,state) VALUES(?,?,?,?,?,?,?,?,?,?)","12121212121212121212121212121212","JUNIOR_TEST","初中测试词书","k12","junior","用于闭环测试",1,1,1,"active");jdbc.update("INSERT INTO vocabulary_book_word(id,book_id,content_id,sort_no,importance,is_core) VALUES(?,?,?,?,?,?)","13131313131313131313131313131313","12121212121212121212121212121212","44444444444444444444444444444444",1,5,1);}
    private void seedAction(String ownerId,LocalDate date){jdbc.update("INSERT INTO weekly_action (id,owner_id,summary_id,action_no,title,note,scheduled_date,estimated_minutes,state,version_no) VALUES (?,?,?,?,?,?,?,?,?,?)","77777777777777777777777777777777",ownerId,"88888888888888888888888888888888",1,"整理今日笔记","从复盘中提炼",date,1,"confirmed",1);}
