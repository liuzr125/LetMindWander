...
public class AdminAuthService {
    ...
    private Map<String,Object> sessionView(String token,Instant expires,AdminPrincipal principal){Map<String,Object> out=new LinkedHashMap<String,Object>();if(token!=null)out.put("token",token);if(expires!=null)out.put("expiresAt",expires);out.put("username",principal.getUsername());out.put("roleCode",principal.getRoleCode());out.put("roleName",principal.getRoleName());out.put("menus",new ArrayList<String>(principal.getMenuCodes()));out.put("menuItems",menus());return out;}
    private List<Map<String,Object>> menus(){return jdbc.queryForList("SELECT id,code,name,path,icon,sort_order sortOrder,parent_id parentId,description FROM admin_menu WHERE enabled=1 ORDER BY sort_order,id");}
