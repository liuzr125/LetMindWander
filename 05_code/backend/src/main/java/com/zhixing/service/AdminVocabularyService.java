...
public class AdminVocabularyService {
    ...
    public AdminVocabularyWordPageView page(String bookId,Integer rawPage,Integer rawPageSize,String rawKeyword){
        ...
        int total=words.countWords(bookId,keyword),totalPages=total==0?0:(total+pageSize-1)/pageSize;
        AdminVocabularyWordPageView result=new AdminVocabularyWordPageView();result.setBook(book);result.setTotal(total);result.setPage(page);result.setPageSize(pageSize);result.setTotalPages(totalPages);result.setKeyword(keyword);result.setItems(words.selectWords(bookId,keyword,(page-1)*pageSize,pageSize));return result;
    }
