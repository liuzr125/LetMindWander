...
public class VocabularyImportService {
    ...
    public Map<String,Object> start(String id, String adminId) {
        ...
        try { rows = parse(string(source,"source_payload")); } catch (ApiException e) { markFailed(id,e.getCode(),e.getMessage()); throw e; }
    ...
    public Map<String,Object> review(String batchId, List<String> itemIds, String decision, String adminId) {
        ...
        for(String itemId:itemIds){Map<String,Object> item=one("SELECT * FROM vocabulary_import_item WHERE id=? AND batch_id=?",itemId,batchId);if(item==null)throw fail(HttpStatus.NOT_FOUND,"IMPORT_ITEM_NOT_FOUND","待审核项不存在");if(!"review".equals(string(item,"decision")))continue;
            if("approve".equals(decision)){String normalized=string(item,"normalized_word");if(normalized==null)throw fail(HttpStatus.CONFLICT,"INVALID_WORD_REQUIRES_FIX","无效词头不能直接通过，需要修正数据源后重新导入");jdbc.update("UPDATE vocabulary_import_item SET decision='create',review_status='approved',error_code=NULL,error_message=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=?",itemId);createTtsTasks(batchId,itemId,normalized,readMap(string(item,"raw_json")),readMap(string(batch,"options_json")));}
    ...
    public Map<String,Object> retryTts(String id,String adminId){
        Map<String,Object> batch=one("SELECT b.*,d.source_url,d.license_note FROM vocabulary_import_batch b JOIN vocabulary_dataset_catalog d ON d.id=b.dataset_id WHERE b.id=? FOR UPDATE",id);
        if(batch==null)throw fail(HttpStatus.NOT_FOUND,"IMPORT_BATCH_NOT_FOUND","词库批次不存在");
        ...
        List<Map<String,Object>> tasks=jdbc.queryForList("SELECT t.*,i.raw_json,i.word_term FROM tts_generation_task t JOIN vocabulary_import_item i ON i.id=t.item_id WHERE t.batch_id=? AND t.state IN ('pending','failed') ORDER BY t.created_at",id);
        for(Map<String,Object> task:tasks){String taskId=string(task,"id");jdbc.update("UPDATE tts_generation_task SET state='running',attempt_count=attempt_count+1,error_code=NULL,error_message=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=?",taskId);try{
                Map<String,Object> raw=readMap(string(task,"raw_json"));String text="example".equals(string(task,"target_type"))?nullable(raw.get("example")):string(task,"word_term");
        ...
        jdbc.update("UPDATE vocabulary_import_batch SET state=?,current_step=?,updated_at=CURRENT_TIMESTAMP WHERE id=?",bad==0?"ready_to_publish":"tts_failed",bad==0?"audio_complete":"tts",id);
    ...
    private void publishItems(String batchId,Map<String,Object> batch,String adminId){
        ...
        String sourceId=CryptoUtils.randomId();jdbc.update("INSERT INTO content_source(id,name,source_type,url,license_note,enabled) VALUES(?,?,?,?,?,1)",sourceId,string(dataset,"dataset_name"),string(dataset,"source_type"),string(dataset,"source_url"),string(dataset,"license_note"));
        String stage=string(one("SELECT level_code FROM vocabulary_book WHERE id=?",string(batch,"target_book_id")),"level_code");int sort=0;
        for(Map<String,Object> item:jdbc.queryForList("SELECT * FROM vocabulary_import_item WHERE batch_id=? AND decision IN ('create','reuse') AND review_status='approved' ORDER BY row_no",batchId)){
            String contentId=string(item,"content_id");if("create".equals(string(item,"decision"))){Map<String,Object> raw=readMap(string(item,"raw_json"));String term=string(item,"normalized_word"),meaning=nullable(raw.get("meaning"));if(meaning==null)throw fail(HttpStatus.CONFLICT,"IMPORT_MEANING_REQUIRED","正式发布前每个新词必须包含释义");
                contentId=CryptoUtils.randomId();String version=CryptoUtils.randomId();jdbc.update("INSERT INTO learning_content(id,content_type,source_id,dedup_hash,word_key_hash,stage,state,current_version_id,published_version_id,published_at) VALUES(?, 'word', ?,?,?,?,'published',?,?,CURRENT_TIMESTAMP)",contentId,sourceId,CryptoUtils.sha256("import:"+batchId+":"+term),CryptoUtils.sha256(term),stage,version,version);
                String example=nullable(raw.get("example")),translation=nullable(raw.get("translation")),phonetic=nullable(raw.get("phonetic"));jdbc.update("INSERT INTO content_version(id,content_id,version_no,title,summary,difficulty,estimated_seconds,word_term,phonetic,meaning,example_text,example_translation,origin_url,license_snapshot,body_hash,review_status,created_by) VALUES(?,?,1,?,?, 'intro',90,?,?,?,?,?,?,?,?,'approved',?)",version,contentId,term,meaning,term,phonetic,meaning,example,translation,string(dataset,"source_url"),string(dataset,"license_note"),CryptoUtils.sha256(write(raw)),adminId);
                String sense=CryptoUtils.randomId();jdbc.update("INSERT INTO word_sense(id,content_version_id,part_of_speech,meaning,sort_no) VALUES(?,?,?, ?,1)",sense,version,nonBlank(raw.get("pos"),"unknown"),meaning);String exampleId=null;if(example!=null){exampleId=CryptoUtils.randomId();jdbc.update("INSERT INTO word_example(id,sense_id,sentence,translation,sort_no) VALUES(?,?,?,?,1)",exampleId,sense,example,translation==null?"":translation);}
                for(Map<String,Object> task:jdbc.queryForList("SELECT * FROM tts_generation_task WHERE item_id=? AND state='succeeded'",string(item,"id"))){boolean ex="example".equals(string(task,"target_type"));jdbc.update("INSERT INTO pronunciation(id,content_version_id,sense_id,example_id,target_key,accent,asset_id,state) VALUES(?,?,?,?,?,?,?,'ready')",CryptoUtils.randomId(),version,ex?null:sense,ex?exampleId:null,(ex?"example":"word")+":1",string(task,"accent"),string(task,"asset_id"));}
    ...
    private void createTtsTasks(String batchId,String itemId,String word,Map<String,Object> raw,Map<String,Object> options){
        String prefix="english/audio/import/"+batchId+"/words/"+objectPart(word)+"/sense-1/";String us=nonBlank(options.get("usVoice"),"eva"),uk=nonBlank(options.get("ukVoice"),"luna");
        task(batchId,itemId,"sense","us",us,prefix+"us.mp3");task(batchId,itemId,"sense","uk",uk,prefix+"uk.mp3");
        if(nullable(raw.get("example"))!=null){String ex="english/audio/import/"+batchId+"/examples/"+objectPart(word)+"/sense-1/example-1/";task(batchId,itemId,"example","us",us,ex+"us.mp3");task(batchId,itemId,"example","uk",uk,ex+"uk.mp3");}
    }
    private void task(String batchId,String itemId,String target,String accent,String voice,String key){try{jdbc.update("INSERT INTO tts_generation_task(id,batch_id,item_id,target_type,accent,provider_code,voice,object_key) VALUES(?,?,?,? ,?,'aliyun_nls',?,?)",CryptoUtils.randomId(),batchId,itemId,target,accent,voice,key);}catch(DuplicateKeyException ignored){}}
