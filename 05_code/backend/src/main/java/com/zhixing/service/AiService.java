...
public class AiService {
    ...
    /** Article help is system content, not a user's Ask question; no personal AI authorization is required. */
    public String generateSystemText(String systemPrompt,String prompt,String requestKey,String sourceId,String actionCode,String sourceType){
        Reservation reservation=transactions.execute(status->reserveSystem(prompt,requestKey,sourceId,actionCode,sourceType));
        try{
    ...
    protected Reservation reserve(String ownerId,String requestedModel,String question,String requestKey){
        ...
        LocalDate today=LocalDate.now(BUSINESS_ZONE),month=today.withDayOfMonth(1);ensureQuota(today,ownerId,personalDailyLimit());ensureQuota(today,GLOBAL_SCOPE,globalDailyLimit());ensureGuard(ownerId,properties.getAi().getPersonalConcurrency());ensureGuard(GLOBAL_SCOPE,properties.getAi().getGlobalConcurrency());
    ...
    protected Reservation reserveSystem(String prompt,String requestKey,String sourceId,String actionCode,String sourceType){
        ...
        if(model.inputPerMillion.signum()==0&&model.outputPerMillion.signum()==0)throw new ApiException(HttpStatus.SERVICE_UNAVAILABLE,"AI_PRICE_NOT_CONFIGURED","模型价格尚未核实，暂时无法使用短文 AI 功能");
        LocalDate today=LocalDate.now(BUSINESS_ZONE),month=today.withDayOfMonth(1);
        ensureQuota(today,SYSTEM_SCOPE,globalDailyLimit());ensureQuota(today,GLOBAL_SCOPE,globalDailyLimit());
        ...
        if(budgets.isEmpty())throw new ApiException(HttpStatus.SERVICE_UNAVAILABLE,"AI_BUDGET_NOT_CONFIGURED","本月 AI 预算尚未配置，暂时无法使用短文 AI 功能");
        ...
        if(jdbc.update("UPDATE ai_month_budget SET reserved_amount=reserved_amount+?,row_version=row_version+1,updated_at=CURRENT_TIMESTAMP WHERE id=? AND spent_amount+reserved_amount+?<=limit_amount",reserved,string(budget,"id"),reserved)!=1)
            throw new ApiException(HttpStatus.TOO_MANY_REQUESTS,"AI_MONTH_BUDGET_EXCEEDED","本月 AI 预算已用完，暂时无法使用短文 AI 功能");
        String jobId=CryptoUtils.randomId(),attemptId=CryptoUtils.randomId(),key=requestKey==null||requestKey.trim().isEmpty()?CryptoUtils.randomToken(18):requestKey.trim();Instant now=Instant.now();
        try{jdbc.update("INSERT INTO ai_job(id,owner_id,scope_key,action_code,source_type,source_id,source_version,source_fingerprint,consent_version,prompt_version,input_text,state,attempt_count,queue_expires_at,payload_expires_at,request_key_hash) VALUES(?,?,?,?,?,?,1,?,'SYSTEM_CONTENT_V1',?,?,'running',1,?,?,?)",jobId,properties.getAdminPrincipalId(),SYSTEM_SCOPE,actionCode,sourceType,sourceId==null?CryptoUtils.randomId():sourceId,CryptoUtils.sha256(prompt),"article_word_glossary".equals(actionCode)?"ARTICLE_WORD_GLOSSARY_V1":("explain_article_paragraph".equals(actionCode)?"ARTICLE_EXPLANATION_V1":"ARTICLE_DAILY_V1"),prompt,Timestamp.from(now.plusSeconds(600)),Timestamp.from(now.plus(Duration.ofDays(30))),CryptoUtils.sha256(key));}
        catch(DuplicateKeyException e){throw conflict("AI_REQUEST_DUPLICATE","该系统 AI 请求正在处理或已处理");}
    ...
    /** 每日额度按「系统参数」当前值建行并同步，改参数当天即生效（不再依赖 yml）。 */
    private void ensureQuota(LocalDate date,String scope,int limit){if(count("SELECT COUNT(*) FROM ai_daily_quota WHERE quota_date=? AND scope_key=?",date,scope)==0)try{jdbc.update("INSERT INTO ai_daily_quota(id,quota_date,scope_key,limit_count,used_count,reserved_count) VALUES(?,?,?,?,0,0)",CryptoUtils.randomId(),date,scope,limit);}catch(DuplicateKeyException ignored){}else jdbc.update("UPDATE ai_daily_quota SET limit_count=?,updated_at=CURRENT_TIMESTAMP WHERE quota_date=? AND scope_key=? AND limit_count<>?",limit,date,scope,limit);}
    private int personalDailyLimit(){return parameters.intValue(AppParameterService.AI_PERSONAL_DAILY_LIMIT,properties.getAi().getPersonalDailyLimit(),1,10000);}
    private int globalDailyLimit(){return parameters.intValue(AppParameterService.AI_GLOBAL_DAILY_LIMIT,properties.getAi().getGlobalDailyLimit(),1,100000);}
