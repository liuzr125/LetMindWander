...
public class AdminContentService {
    ...
    public AdminTechnicalContentPageView technical(String rawTopic,String rawKeyword,Integer rawPage,Integer rawPageSize){
        ...
        List<AdminTechnicalContentView> items=contents.selectTechnical(topic,keyword,(safePage-1)*pageSize,pageSize);
        for(AdminTechnicalContentView item:items)item.setTopics(contents.selectTechnicalTopicNames(item.getVersionId()));
        AdminTechnicalContentPageView result=new AdminTechnicalContentPageView();result.setTotal(total);result.setPage(safePage);result.setPageSize(pageSize);result.setTotalPages(totalPages);result.setTopic(topic);result.setKeyword(keyword);result.setItems(items);return result;
    }
    ...
    public AdminTechnicalContentPageView articles(String rawDifficulty,String rawBookId,String rawKeyword,Integer rawPage,Integer rawPageSize){return articles(rawDifficulty,rawBookId,rawKeyword,"",rawPage,rawPageSize);}
    public AdminTechnicalContentPageView articles(String rawDifficulty,String rawBookId,String rawKeyword,String rawAudioStatus,Integer rawPage,Integer rawPageSize){
        int page=rawPage==null?1:rawPage,pageSize=rawPageSize==null?20:rawPageSize;
        if(page<1||pageSize<1||pageSize>100)throw new ApiException(HttpStatus.BAD_REQUEST,"INVALID_PAGE","页码从 1 开始，每页可显示 1 至 100 条内容");
        String difficulty=clean(rawDifficulty).toLowerCase(java.util.Locale.ROOT),bookId=clean(rawBookId),keyword=clean(rawKeyword),audioStatus=clean(rawAudioStatus).toLowerCase(java.util.Locale.ROOT);
        ...
        if(!audioStatus.isEmpty()&&!"has_audio".equals(audioStatus)&&!"missing_audio".equals(audioStatus))throw new ApiException(HttpStatus.BAD_REQUEST,"INVALID_AUDIO_FILTER","音频筛选值不正确");
        List<AdminTechnicalContentView> all=contents.selectArticles(difficulty,bookId,keyword,audioStatus);
        ...
        int safePage=totalPages==0?1:Math.min(page,totalPages);
        int from=Math.min((safePage-1)*pageSize,total),to=Math.min(from+pageSize,total);
        ...
        AdminTechnicalContentPageView result=new AdminTechnicalContentPageView();result.setTotal(total);result.setPage(safePage);result.setPageSize(pageSize);result.setTotalPages(totalPages);result.setTopic(difficulty);result.setKeyword(keyword);result.setItems(items);return result;
    }
