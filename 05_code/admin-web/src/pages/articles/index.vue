  ...
  <section>
    ...
    <template v-else>
      ...
      <div class="panel article-panel">
        ...
        <div class="table-wrap"><table><thead><tr><th>标题</th><th>难度</th><th>阅读时长</th><th>来源</th><th>来源日期</th><th>操作</th></tr></thead><tbody><tr v-for="item in articles.items" :key="item.contentId"><td><strong>{{ item.title }}</strong><small>{{ item.summary || '暂无摘要' }}</small></td><td><span class="difficulty">{{ label(item.difficulty) }}</span></td><td>{{ Math.max(1,Math.ceil((item.estimatedSeconds||0)/60)) }} 分钟</td><td>{{ item.sourceName }}</td><td>{{ date(item.originPublishedAt || item.publishedAt) }}</td><td><button class="link" @click="openDetail(item.contentId)">查看详情 ›</button></td></tr><tr v-if="!listLoading&&!articles.items.length"><td colspan="6" class="empty">没有符合条件的英语短文</td></tr></tbody></table><div v-if="listLoading" class="loading">正在加载…</div></div>
