...
const ownerId = computed(() => route.params.ownerId), bookId = computed(() => route.params.bookId)
const loading = ref(true), error = ref('')
const detail = ref({ record: null, days: [], activeDays: 0 })
const words = ref({ items: [], total: 0, page: 1, pageSize: 20, totalPages: 0 })
const status = ref('learned'), wordPage = ref(1), wordPageSize = ref(20)
...
function userId() { return value(record.value, 'shortId') || ownerId.value }
...
async function load() {
  ...
  try { detail.value = await request(`/admin/study-records/${encodeURIComponent(ownerId.value)}/${encodeURIComponent(bookId.value)}`) }
  catch (e) { error.value = e.message || '读取学习记录详情失败' } finally { loading.value = false }
}
async function loadWords(target = wordPage.value) {
  try {
    const params = new URLSearchParams({ page: target, pageSize: wordPageSize.value, status: status.value })
    words.value = await request(`/admin/study-records/${encodeURIComponent(ownerId.value)}/${encodeURIComponent(bookId.value)}/words?${params.toString()}`)
    wordPage.value = words.value.page
  } catch (e) { error.value = e.message || '读取已学词条失败' }
}
  ...
  <section>
    <div class="page-heading">
      <div>
        ...
        <h1>学习记录详情</h1>
        <p>{{ userName() }} · {{ bookTitle() }}：这本书的学习时长、每日明细与已学词条。</p>
      </div>
      <button class="refresh" :disabled="loading" @click="load(); loadWords(wordPage)">刷新数据</button>
    </div>
    <div v-if="loading && !record.id && !record.ownerId" class="panel state">正在读取学习记录…</div>
    <div v-else-if="error && !record.ownerId" class="panel state">{{ error }}</div>
    <template v-else>
      <div class="panel summary-panel">
        <div class="summary-head">
          <div><h2>{{ bookTitle() }}</h2><p>{{ userName() }} · <span class="code">{{ userId() }}</span> · {{ mobile() }}</p></div>
          <div class="tags"><span class="difficulty">{{ level() }}</span><span v-if="value(record, 'currentBook')" class="pill current">当前词书</span></div>
        </div>
        <div class="progress-row">
          <div class="progress-main"><strong>{{ num(value(record, 'learnedWords')) }} / {{ num(value(record, 'totalWords')) }}</strong><span>已学 / 词书总词数</span></div>
      ...
      <div class="panel record-panel">
        <div class="panel-head"><div><h2>每日学习明细</h2><p>共 {{ num(detail.activeDays) }} 天有学习动作 · 下表为最近 {{ detail.days.length }} 行（含仅有新学记录的日子）</p></div></div>
      ...
      <div class="panel record-panel">
        <div class="panel-head">
          <div><h2>这本书的词条</h2><p>{{ statusText(status) }} · 共 {{ num(words.total) }} 个</p></div>
          ...
          <table>
            ...
            <tbody>
              <tr v-for="row in words.items" :key="value(row, 'contentId')">
                ...
                <td><span class="pill" :class="value(row, 'learningStatus') === 'learning' ? 'open' : 'done'">{{ value(row, 'statusLabel') }}</span></td>
              ...
              <tr v-if="!words.items.length"><td colspan="9" class="empty">该状态下没有词条</td></tr>
            </tbody>
          </table>
        </div>
