...
<template>
  <section>
    <div class="page-heading"><div><h1>内容与来源</h1><p>核对技术主题的初始化覆盖率，发布内容仍需保留来源与许可。</p></div><button class="refresh" @click="load">刷新数据</button></div>
    <div v-if="loading" class="panel state">正在读取内容…</div>
    <div v-else-if="error" class="panel state"><strong>数据暂未加载</strong><button class="retry" @click="load">重新加载</button></div>
    <template v-else>
      ...
      <div class="panel compact-selector">
        <label><span>技术知识主题</span><select :value="topic" @change="chooseTopic($event.target.value)"><option value="">全部主题</option><option v-for="item in coverage.technicalTopics" :key="item.name" :value="item.name">{{ item.name }}</option></select></label>
        <div class="selector-summary"><strong>{{ topic || '全部主题' }}</strong><span v-if="topic">已发布 {{ coverage.technicalTopics.find(item=>item.name===topic)?.itemCount || 0 }} 条</span><span v-else>共 {{ coverage.technicalTopics.length }} 个主题 · {{ coverage.publishedTotal }} 条已发布内容</span><em v-if="topic" :class="(coverage.technicalTopics.find(item=>item.name===topic)?.itemCount||0)>=25?'ok':'bad'">{{ (coverage.technicalTopics.find(item=>item.name===topic)?.itemCount||0)>=25?'达标':'不足' }}</em></div>
      </div>
      ...
      <div class="panel knowledge-panel">
        ...
        <div class="content-table"><table><thead><tr><th>标题</th><th>主题</th><th>难度</th><th>来源</th><th>来源日期</th><th></th></tr></thead><tbody><tr v-for="item in technical.items" :key="item.contentId"><td><strong>{{ item.title }}</strong><small>{{ item.summary || '暂无摘要' }}</small></td><td><span v-for="name in item.topics" :key="name" class="topic-tag">{{ name }}</span></td><td>{{ difficulty(item.difficulty) }}</td><td>{{ item.sourceName }}</td><td>{{ date(item.originPublishedAt || item.publishedAt) }}</td><td><button class="link" @click="openDetail(item.contentId)">查看详情 ›</button></td></tr><tr v-if="!techLoading && !technical.items.length"><td colspan="6" class="empty">没有符合条件的技术知识</td></tr></tbody></table><div v-if="techLoading" class="loading-mask">正在加载…</div></div>
