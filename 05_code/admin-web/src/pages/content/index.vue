...
<template>
  ...
  <div v-if="detailOpen" class="modal-mask" @click.self="closeDetail"><article class="detail-modal"><header><div><span>技术知识详情</span><h2>{{ detail?.title || '正在加载…' }}</h2></div><button @click="closeDetail">×</button></header><div v-if="detailLoading" class="detail-state">正在读取详情…</div><div v-else-if="detailError" class="detail-state error">{{ detailError }}</div><template v-else-if="detail"><div class="detail-meta"><span>{{ difficulty(detail.difficulty) }}</span><span>预计 {{ Math.max(1, Math.ceil(detail.estimatedSeconds / 60)) }} 分钟</span><span>版本 {{ detail.versionNo }}</span><span>{{ detail.reviewStatus === 'approved' ? '已审核' : detail.reviewStatus }}</span></div><div class="detail-topics"><span v-for="name in detail.topics" :key="name">{{ name }}</span></div><section v-if="detail.summary"><h3>摘要</h3><p>{{ detail.summary }}</p></section><section><h3>正文</h3><div class="body">{{ detail.body || '暂无正文' }}</div></section><section class="source-box"><h3>来源与许可</h3><dl><div><dt>内容来源</dt><dd>{{ detail.sourceName }}（{{ detail.sourceType }}）</dd></div><div><dt>原作者</dt><dd>{{ detail.originAuthor || '—' }}</dd></div><div><dt>原文发布时间</dt><dd>{{ date(detail.originPublishedAt) }}</dd></div><div><dt>站内发布时间</dt><dd>{{ date(detail.publishedAt) }}</dd></div><div><dt>许可快照</dt><dd>{{ detail.licenseSnapshot }}</dd></div></dl><a v-if="detailUrl" :href="detailUrl" target="_blank" rel="noopener noreferrer">打开原始链接 ↗</a></section></template></article></div>
</template>
