...
<template>
  <section>
    ...
    <div class="book-strip">
      <button v-for="book in books" :key="book.bookId" :class="['book-card',{active:book.bookId===selectedBookId}]" @click="chooseBook(book.bookId)">
        <span>{{ book.levelCode || book.bookType }}</span><strong>{{ book.bookName }}</strong><small>成员 {{ book.memberCount }} · 可用 {{ book.availableCount }}</small><em v-if="book.countMismatch">标称 {{ book.declaredWordCount }}，数量待同步</em>
      </button>
      <div v-if="!books.length && !loading" class="panel empty-book">暂无启用词书</div>
    </div>
