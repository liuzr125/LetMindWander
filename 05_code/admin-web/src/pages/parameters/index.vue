...
const NLS_TOKEN_KEY = 'ALIYUN_NLS_TEMPORARY_TOKEN'
const preparingToken = ref(false)
async function openNlsToken() {
  if (preparingToken.value) return
  preparingToken.value = true
  try {
    const result = await request('/admin/parameters?' + new URLSearchParams({ keyword: NLS_TOKEN_KEY, pageSize: '100' }))
    const existing = (result.items || []).find(item => item.paramKey === NLS_TOKEN_KEY)
    if (existing) openEdit(existing)
    else { openCreate(); form.value.paramKey = NLS_TOKEN_KEY }
    form.value.isSecret = true; form.value.state = 'active'
    form.value.description = 'NLS 临时 Token；后端为小程序按需生成音频，过期后需更新'
  } catch (e) { fail(e) }
  finally { preparingToken.value = false }
}
function closeDialog() { dialogOpen.value = false; form.value.paramValue = '' }
watch(() => form.value.paramKey, key => { if (key.toUpperCase() === NLS_TOKEN_KEY) form.value.isSecret = true })
...
<template>
  <section>
    ...
    <div class="panel token-panel"><div><h2>小程序语音临时 Token</h2><p>参数键：<code>ALIYUN_NLS_TEMPORARY_TOKEN</code>。加密保存后，后端自动读取用于短文与单词发音，音频上传 OSS 并保存数据库；Token 不下发小程序。过期后在此更新，重启无需重新输入。</p><p>保存仅代表已配置，不延长阿里云 Token 有效期，也不验证其与 AppKey 是否匹配。</p></div><button class="primary" :disabled="preparingToken" @click="openNlsToken">{{ preparingToken ? '正在读取…' : '保存 / 更新临时 Token' }}</button></div>
  ...
  <div v-if="dialogOpen" class="modal-mask" @click.self="closeDialog"><form class="dialog" @submit.prevent="submit"><h2>{{ editing?'编辑系统参数':'新增系统参数' }}</h2><p v-if="editing&&form.isSecret" class="hint">敏感参数已加密保存；不填写参数值将保留现有值。</p><label>参数键<input v-model="form.paramKey" :disabled="editing" maxlength="100" placeholder="例如 tts.aliyun.voice" required /></label><label>参数值<input autocomplete="off" v-model="form.paramValue" :type="form.isSecret?'password':'text'" maxlength="4096" :placeholder="editing&&form.isSecret?'留空表示不修改':'请输入参数值'" :required="!editing || !form.isSecret" /></label><label class="check"><input v-model="form.isSecret" :disabled="form.paramKey.toUpperCase()===NLS_TOKEN_KEY" type="checkbox" />敏感参数（加密保存，不能回显）</label><label>参数说明<input v-model="form.description" maxlength="200" required /></label><label>状态<select v-model="form.state"><option value="active">启用</option><option value="inactive">停用</option></select></label><div class="form-actions"><button type="button" @click="closeDialog">取消</button><button class="primary" type="submit">{{ editing?'保存修改':'新增参数' }}</button></div></form></div>
</template>

<style scoped>
.token-panel{display:flex;gap:20px;align-items:center;justify-content:space-between}.token-panel h2{margin:0;font-size:18px}.token-panel p{color:#65738d;font-size:13px;line-height:1.7}.token-panel button{padding:12px 16px;border-radius:8px;white-space:nowrap}@media(max-width:900px){.token-panel{align-items:stretch;flex-direction:column}}
