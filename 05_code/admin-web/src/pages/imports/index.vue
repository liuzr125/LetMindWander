...
async function deleteBatch(batch) {
  ...
  if (!window.confirm(`确定删除批次“${batch.datasetName}”（${String(batch.batchId).slice(0,12)}）吗？\n将一并删除该批次的暂存词条与语音任务；已发布的批次不能删除。`)) return
  deletingBatch.value = batch.batchId
  error.value = ''
  try {
    const result = await request(`/admin/vocabulary/import-batches/${encodeURIComponent(batch.batchId)}`, { method:'DELETE' })
    if (selected.value && selected.value.batchId === batch.batchId) selected.value = null
    await loadAll()
    flash(`批次已删除，清理暂存词条 ${result.removedItems ?? 0} 条`)
  } catch (e) { failure(e) } finally { deletingBatch.value = '' }
}
...
<template>
  <section>
    ...
    <div class="panel batch-panel"><div class="panel-title"><div><h2>批次执行中心</h2><p>启动批次会解析、规范化、识别重复词，并生成稳定 object_key 的语音任务计划。</p></div></div><div class="table-wrap"><table><thead><tr><th>批次</th><th>目标词书</th><th>状态</th><th>步骤</th><th>结果</th><th>更新时间</th><th></th></tr></thead><tbody><tr v-for="batch in batches" :key="batch.batchId"><td><strong>{{ batch.datasetName }}</strong><small>{{ batch.batchId.slice(0,12) }}</small></td><td>{{ batch.targetBookName }}</td><td><span :class="['pill',batch.state==='published'?'ok':batch.state==='failed'?'danger':'warn']">{{ stateText(batch.state) }}</span></td><td>{{ batch.currentStep }}</td><td>成功 {{ batch.successCount }} · 新建 {{ batch.createdCount }} · 复用 {{ batch.reusedCount }} · 待审 {{ batch.reviewCount }}</td><td>{{ date(batch.updatedAt) }}</td><td class="batch-actions"><button class="link" @click="openBatch(batch.batchId)">查看</button><button class="link delete-link" :disabled="!!deletingBatch" @click="deleteBatch(batch)">{{ deletingBatch===batch.batchId?'删除中…':'删除' }}</button></td></tr><tr v-if="!batches.length"><td colspan="7" class="empty">尚未创建批次</td></tr></tbody></table></div></div>
